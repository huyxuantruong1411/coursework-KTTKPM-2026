...
class _StatsScreenState extends State<StatsScreen> {
  ...
  Widget _buildContent() {
    ...
    final dailyActivity =
        (s['daily_activity'] as List? ?? []).cast<Map<String, dynamic>>();
    final genreDistribution =
        (s['genre_distribution'] as List? ?? []).cast<Map<String, dynamic>>();
    final themeDistribution =
        (s['theme_distribution'] as List? ?? []).cast<Map<String, dynamic>>();
    final recentManga =
        (s['recent_manga'] as List? ?? []).cast<Map<String, dynamic>>();
      ...
      child: ListView(
        ...
        children: [
          ...
          GridView.count(
            ...
            children: [
              _kpiCard('Manga đã đọc', totalManga.toString(),
                  Icons.menu_book_rounded, _kOrange),
              _kpiCard('Chapter đã đọc', totalChapters.toString(),
                  Icons.bookmark_rounded, const Color(0xFF4CAF50)),
              _kpiCard('Trang đã đọc', _formatNum(totalPages),
                  Icons.article_outlined, const Color(0xFF2196F3)),
              _kpiCard(
                'Đánh giá đã cho',
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  SizedBox(
                    height: 120,
                    child: _ActivityBarChart(data: dailyActivity),
                  ),
...
class _BarChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final maxCount =
        data.map((e) => e['count'] as int? ?? 0).reduce(max).toDouble();
    ...
    // Baseline
    canvas.drawLine(
      Offset(0, size.height - chartPadBottom),
      Offset(size.width, size.height - chartPadBottom),
      basePaint,
    );
    ...
    if (n >= 2) {
      ...
      // First label
      tp.text = TextSpan(
          text: _shortDate(data.first['date']?.toString() ?? ''),
          style: labelStyle);
      tp.layout();
      ...
      // Last label
      tp.text = TextSpan(
          text: _shortDate(data.last['date']?.toString() ?? ''),
          style: labelStyle);
      tp.layout();
      tp.paint(
          canvas,
          Offset(size.width - tp.width,
              size.height - chartPadBottom + 4));
    }
  }
...
class _DistributionList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final maxCount =
        items.map((e) => e['count'] as int? ?? 0).reduce(max).toDouble();
    ...
    return Container(
      ...
      child: Column(
        children: List.generate(items.length, (i) {
          ...
          return Padding(
            padding:
                EdgeInsets.fromLTRB(14, i == 0 ? 12 : 6, 14, i == items.length - 1 ? 12 : 6),
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Row(
